Router.configure({
  ...
  // track page views in Google Analytics by default
  trackPageView: true
});
...
Router.plugin('ensureSignedIn', {
  only: ['newDream', 'newRecipe', 'newActivity']
});
...
Router.route('/dreams', {
  ...
  data: function() {
    return {
      ...
      pickedDreams: orion.entities.dreams.collection.find({
        editorsPick: true
      }, {sort: {createdAt: -1}, limit: Config.editorsPicksToDisplay})
    };
  }
});
...
Router.route('/recipes', {
  ...
  data: function() {
    return {
      ...
      pickedRecipes: orion.entities.recipes.collection.find(
        _.extend({
          editorsPick: true
        }, this.params.query.dream ? {dreamIds: this.params.query.dream} : {}),
        {sort: {createdAt: -1}, limit: Config.editorsPicksToDisplay}
      ),
...
Router.route('/activities', {
  ...
  data: function() {
    return {
      ...
      pickedActivities: orion.entities.activities.collection.find(
        _.extend({
          editorsPick: true
        }, this.params.query.recipe ? {recipeIds: this.params.query.recipe} : {}),
        {sort: {createdAt: -1}, limit: Config.editorsPicksToDisplay}
      ),
